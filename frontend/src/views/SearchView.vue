<template>
    <body> 
        <main class="px-8 py-6 bg-[#badfe7]">
            <div class="max-w-7xl mx-auto grid grid-cols-4 gap-4">
                <div class="main-center col-span-4 space-y-4">
                    <!-- searchbar start-->
                    <div class="bg-[#f6f6f2] border border-gray-200 rounded-lg">
                        <form v-on:submit.prevent="submitForm" class="p-4 flex space-x-4">  
                            <input v-model="query" type="search" class="p-4 w-full bg-white rounded-lg" placeholder="What are you looking for?">

                            <button class="inline-block py-4 px-6 bg-[#c2edce] text-black rounded-lg">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                                </svg>      
                            </button>
                        </form>
                    </div>
                    <!-- searchbar end-->

                    <!-- if may na search na user start-->
                    <div class="p-4 bg-[#f6f6f2] border border-gray-200 rounded-lg grid grid-cols-4 gap-4"
                    v-if="users.length"
                    >
                        <div class="p-4 text-center bg-[#f6f6f2] rounded-lg"
                        v-for="user in users"
                        v-bind:key="user.id"
                        >
                            <img :src="user.get_avatar" class="mb-6 rounded-full">
                        
                            <p><strong><RouterLink :to="{name: 'profile', params:{'id': user.id}}">{{user.name}}</RouterLink></strong></p>

                            <div class="mt-6 flex space-x-8 justify-around">
                                <p class="text-xs text-gray-500">{{user.posts_count}} posts</p>
                            </div>
                        </div>
                    </div>
                    <!-- if may na search na user end -->

                    <!-- if may na search na post start-->
                    <div 
                    class="p-4 bg-white border border-gray-200 rounded-lg"
                    v-for="post in posts"
                    v-bind:key="post.id"
                    >
                        <div class="mb-6 flex items-center justify-between">
                            <div class="flex items-center space-x-6">
                                <RouterLink :to="{name: 'profile', params: {id: post.created_by.id}}">
                                    <img :src="post.created_by.get_avatar" class="w-[40px] rounded-full">
                                </RouterLink>
                                
                                <p><strong><RouterLink :to="{name: 'profile', params:{'id': post.created_by.id}}">{{ post.created_by.name }}</RouterLink></strong></p>
                            </div>

                            <p class="text-gray-600">{{ post.created_at_formatted}}</p>
                        </div>

                        
                        <template v-if="post.attachments.length">
                            <img v-for="image in post.attachments" v-bind:key="image.id" :src="image.get_image" class="w-full mb-4 rounded-xl">
                        </template>
                        <p>{{ post.body }}</p>
                        
                        <div class="my-6 flex justify-between">
                            <div class="flex space-x-6">
                                <div class="flex items-center space-x-2" @click="likePost(post.id)">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                                    </svg>  
                                    
                                    <span class="text-gray-500 text-xs hover:cursor-pointer" >
                                        {{ post.likes_count }}
                                        {{ post.likes_count === 1 || post.likes_count === 0 ? 'like' : 'likes' }}
                                    </span>                             
                                </div> 
                                
                                <div class="flex items-center space-x-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 01-.923 1.785A5.969 5.969 0 006 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337z" />
                                    </svg> 

                                    <RouterLink :to="{name: 'postview', params: {id: post.id}}" class="text-gray-500 text-xs hover:cursor-pointer">{{ post.comments_count }} comments</RouterLink>
                                </div>
                            </div>
                            
                        </div>  
                    </div>
                    <!-- if may na search na post start-->
                </div>

            </div>
        </main>
    </body>
</template>


<script>
    import axios from 'axios'

    export default {
        // Component name
        name: 'SearchView',

        // Component data
        data() {
            return {
                query: '', // Search query
                users: [], // Array to store search results for users
                posts: [], // Array to store search results for posts
            }
        },

        // Component methods
        methods: {
            // Method to submit search form
            submitForm() {
                console.log('submitForm', this.query)

                // Send search query to server
                axios
                    .post('/api/search/', {
                        query: this.query
                    })
                    .then(response => {
                        console.log('response', response.data)

                        // Update users and posts arrays with search results
                        this.users = response.data.users
                        this.posts = response.data.posts
                    })
                    .catch(error => {
                        console.log('error', error)
                    })
            },

            // Method to like a post
            likePost(id) {
                axios
                    .post(`/api/posts/${id}/like/`)
                    .then(response => {
                        console.log(response.data);
                        // Reload the page after the like is added
                        location.reload();
                    })
                    .catch(error => {
                        console.log('error', error)
                    });
            },
        },
    }
</script>
